...
class A4NetworkPrinterService implements BasePrinterService {
  static final A4NetworkPrinterService _instance =
  ...
  Future<PrinterResult> printA4Receipt({
    ...
    _talker.info(
      '[A4Printer: $ipAddress:$port] Starting A4 print job with ${pdfBytes.length} bytes',
    );
    ...
    for (int attempt = 1; attempt <= 2; attempt++) {
      if (attempt > 1) {
        _talker.debug(
          '[A4Printer: $ipAddress] Retry attempt $attempt after 2 second delay',
        );
      ...
      _talker.debug(
        '[A4Printer: $ipAddress] Trying Strategy 1: Pure PDF (Attempt $attempt/2)',
      );
      debugPrint('📄 [A4NetworkPrinter] Trying Strategy 1: Pure PDF');
      final result1 = await _printWithStrategy1(
        ipAddress,
        pdfBytes,
        port,
        timeout,
      ...
      _talker.warning(
        '[A4Printer: $ipAddress] Strategy 1 failed, trying Strategy 2',
      );
      debugPrint(
        '⚠️ [A4NetworkPrinter] Strategy 1 failed, trying Strategy 2: PDF with PCL wrapper',
      );
      // Strategy 2: PDF with PCL wrapper (for printers that need mode switching)
      final result2 = await _printWithStrategy2(
        ipAddress,
        pdfBytes,
        port,
        timeout,
      ...
      _talker.warning(
        '[A4Printer: $ipAddress] Strategy 2 failed, trying Strategy 3',
      );
      debugPrint(
        '⚠️ [A4NetworkPrinter] Strategy 2 failed, trying Strategy 3: PDF with minimal commands',
      );
      // Strategy 3: PDF with minimal commands
      final result3 = await _printWithStrategy3(
        ipAddress,
        pdfBytes,
        port,
        timeout,
    ...
    _talker.error(
      '[A4Printer: $ipAddress] ❌ ALL STRATEGIES FAILED after 2 attempts',
    );
  ...
  Future<PrinterResult> _printWithStrategy1(
    String ipAddress,
    Uint8List pdfBytes,
    int port,
    Duration timeout,
    ...
    try {
      ...
      _talker.info(
        '[A4Printer: $ipAddress] Strategy 1: Sending pure PDF (${pdfBytes.length} bytes)',
      );
      debugPrint(
        '📄 [A4NetworkPrinter] Strategy 1: Sending pure PDF (${pdfBytes.length} bytes)',
      );
      socket.add(pdfBytes);
      ...
      _talker.info(
        '[A4Printer: $ipAddress] ✅ Strategy 1 completed successfully',
      );
      debugPrint('✅ [A4NetworkPrinter] Strategy 1 completed');
      return PrinterResult(
        success: true,
        message: 'A4 receipt printed (Strategy 1)',
        code: 0,
      );
    } catch (e) {
  ...
  Future<PrinterResult> _printWithStrategy2(
    String ipAddress,
    Uint8List pdfBytes,
    int port,
    Duration timeout,
    ...
    try {
      ...
      _talker.info(
        '[A4Printer: $ipAddress] Strategy 2: Sending PDF with PCL wrapper',
      );
      ...
      _talker.info(
        '[A4Printer: $ipAddress] ✅ Strategy 2 completed successfully',
      );
      debugPrint('✅ [A4NetworkPrinter] Strategy 2 completed');
      return PrinterResult(
        success: true,
        message: 'A4 receipt printed (Strategy 2)',
        code: 0,
      );
    } catch (e) {
  ...
  Future<PrinterResult> _printWithStrategy3(
    String ipAddress,
    Uint8List pdfBytes,
    int port,
    Duration timeout,
    ...
    try {
      ...
      _talker.info(
        '[A4Printer: $ipAddress] Strategy 3: Sending PDF with minimal commands',
      );
      ...
      _talker.info(
        '[A4Printer: $ipAddress] ✅ Strategy 3 completed successfully',
      );
      debugPrint('✅ [A4NetworkPrinter] Strategy 3 completed');
      return PrinterResult(
        success: true,
        message: 'A4 receipt printed (Strategy 3)',
        code: 0,
      );
    } catch (e) {
  ...
  Future<PrinterResult> printThermalReceipt({
    ...
    return PrinterResult(
      success: false,
      message: 'A4 printer does not support thermal receipt format',
      code: -1,
    );
  }
  ...
  Future<PrinterResult> printLabel({
    ...
    try {
      _talker.info(
        '[A4Printer: $ipAddress] Converting text to PDF for label printing',
      );
      debugPrint(
        '📄 [A4NetworkPrinter] Converting text to PDF for label printing',
      );
      ...
      _talker.error(
        '[A4Printer: $ipAddress] ❌ Failed to convert text to PDF: $e',
      );
  ...
  Future<PrinterResult> printDeviceLabel({
    ...
    try {
      _talker.info(
        '[A4Printer: $ipAddress] Creating formatted device label PDF',
      );
      ...
      _talker.error(
        '[A4Printer: $ipAddress] ❌ Failed to create device label: $e',
      );
      debugPrint('❌ [A4NetworkPrinter] Failed to create device label: $e');
      return PrinterResult(
        success: false,
        message: 'Failed to create device label: $e',
        code: -1,
      );
    }
  }
  ...
  Future<PrinterResult> printLabelImage({
    ...
    return PrinterResult(
      success: false,
      message: 'A4 printer does not support label image format',
      code: -1,
    );
  }
  ...
  Future<PrinterResult> printBorderTest({
    ...
    return PrinterResult(
      success: false,
      message: 'Border test not supported for A4',
      code: -2,
    );
  }
  ...
  Future<PrinterResult> calibrate({
    ...
    return PrinterResult(
      success: false,
      message: 'Calibration not supported for A4',
      code: -2,
    );
  }
}
