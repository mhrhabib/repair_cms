...
class BrotherPrinterService implements BasePrinterService {
  ...
  Future<PrinterResult> _printViaTDRaster(String ipAddress, String text, Duration timeout) async {
    try {
      ...
      final bytes = _buildTDRasterCommands(text, labelWidth, labelHeight, dotsPerMm, modelString);
  ...
  Future<PrinterResult> printLabelImage({
    ...
    try {
      ...
      final bytes = await _buildTDRasterFromImage(uiImage, labelWidth, labelHeight, dotsPerMm, modelString);
  ...
  List<int> _buildTDRasterCommands(String text, int labelWidth, int labelHeight, double dotsPerMm, String modelString) {
    final List<int> bytes = [];
    const esc = 0x1B;
  ...
  Future<List<int>> _buildTDRasterFromImage(
    ...
    // right by ~81% of the gap to align with the label's printable area.
    final int totalGap = printHeadLineBytes - imageLineBytes;
    final int leftPadBytes = isTD4 ? (totalGap * 0.81).round() : 0;
    ...
    _talker.info('[BrotherRawTCP] 📊 Mapping strategy: Standard Portrait Y-flip (${isTD4 ? "TD-4" : "TD-2D"})');
  ...
  @override
  Future<PrinterResult> printBorderTest({required String ipAddress, int port = 9100, LabelSize? labelSize}) async {
    return PrinterResult(success: false, message: 'Border test not implemented for raw Brother', code: -2);
  }

  @override
  Future<PrinterResult> calibrate({required String ipAddress, int port = 9100}) async {
    return PrinterResult(success: false, message: 'Calibration not implemented for raw Brother', code: -2);
  }
}
