...
class JobRepository {
  Future<JobListResponse> getJobs({
    ...
    try {
      final Map<String, dynamic> queryParams = {
        ...
        'assignUserId': (assignee != null && assignee != 'None')
            ? assignee
            : '',
        'job_priority':
            (priority != null && priority.isNotEmpty && priority != 'All')
            ? priority
            : 'null',
      ...
      if (response.statusCode == 200) {
        ...
        throw Exception(
          'Failed to fetch jobs: ${response.statusCode} - ${response.data}',
        );
      }
  ...
  Future<SingleJobModel> updateJobCompletionStatus(
    ...
    try {
      ...
      final List<Map<String, dynamic>> existingStatuses =
          currentJob.data?.jobStatus?.map((s) => s.toJson()).toList() ??
          <Map<String, dynamic>>[];
  ...
  Future<SingleJobModel> updateJobReturnStatus(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(
          '✅ Response - is_device_returned: ${responseData['data']?['is_device_returned']}',
        );
  ...
  Future<SingleJobModel> updateJobPriority(
    ...
    try {
      ...
      debugPrint(
        '🔄 repo: Updated job priority for Job ID: ${responseData['data']?['_id']} to $priority',
      );
  ...
  Future<SingleJobModel> uploadJobFile({
    ...
    try {
      ...
      if (isImage) {
        try {
          debugPrint(
            '📸 [uploadJobFile] Fixing orientation for image: $fileName',
          );
          ...
          if (decodedImage != null) {
            ...
            debugPrint('✅ [uploadJobFile] Orientation fixed');
          }
        } catch (e) {
          debugPrint('⚠️ [uploadJobFile] Orientation fix failed: $e');
        }
      }
      ...
      final uploadResponse = await BaseClient.post(
        url: uploadUrl,
      ...
      final uploadedFilePath = uploadResponse.data is String
          ? uploadResponse.data as String
          : null;
      ...
      try {
        final imageUrl =
            '${ApiEndpoints.fileUplaodUrl}images?imagePath=$uploadedFilePath';
      ...
      final patchResponse = await BaseClient.patch(
        url: patchUrl,
      ...
      final responseData = patchResponse.data is String
          ? jsonDecode(patchResponse.data)
          : patchResponse.data;
      ...
      if (responseData is Map && responseData['data'] != null) {
          ...
          for (var fileData in files) {
            if (fileData is Map &&
              ...
              try {
                // Get signed URL
                final imageUrl =
                    '${ApiEndpoints.fileUplaodUrl}images?imagePath=$filePath';
                debugPrint('📥 Fetching signed URL: $imageUrl');

                final urlResponse = await BaseClient.get(url: imageUrl);

                if (urlResponse.statusCode == 200 &&
                    urlResponse.data is String) {
                  fileData['url'] = urlResponse.data as String;
                  debugPrint('✅ Got signed URL for: ${fileData['fileName']}');
                }
              } catch (e) {
                debugPrint(
                  '⚠️ Could not get signed URL for ${fileData['fileName']}: $e',
                );
              }
            }
          }
        ...
        return SingleJobModel.fromJson({
          'success': true,
          'data': Map<String, dynamic>.from(responseData['job']),
        });
      } else {
  ...
  Future<SingleJobModel> deleteJobFile({
    ...
    try {
      ...
      if (patchResponse.statusCode == 200 || patchResponse.statusCode == 201) {
        ...
        throw Exception(
          'Failed to update job file list: ${patchResponse.statusCode}',
        );
      }
    } catch (e, stackTrace) {
  ...
  Future<StatusSettingsResponse> getStatusSettings(String userId) async {
    try {
      debugPrint(
        '🚀 [JobRepository] Fetching status settings for user: $userId',
      );
      ...
      if (response.statusCode == 200) {
        ...
        debugPrint(
          '📊 [JobRepository] Total statuses: ${responseData['totalStatus'] ?? 0}',
        );

        return StatusSettingsResponse.fromJson(responseData);
      } else {
        throw Exception(
          'Failed to fetch status settings: ${response.statusCode} - ${response.data}',
        );
      }
    } on dio.DioException catch (e) {
