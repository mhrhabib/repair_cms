...
class JobCreateCubit extends Cubit<JobCreateState> {
  ...
  Future<void> updateJob({required CreateJobRequest request, required String jobId}) async {
    emit(JobCreateLoading());

    try {
      debugPrint('🚀 [JobCubit] Starting job update...');
      // Implement update logic here
      final response = await jobRepository.updateJob(request: request, jobId: jobId);

      emit(JobCreateCreated(response: response));

      debugPrint('✅ [JobCubit] Job updated successfully');
      // emit(JobCreateUpdated(response: response));
    } on JobException catch (e) {
      debugPrint('❌ [JobCubit] Job update failed: ${e.message}');
      emit(JobCreateError(message: e.message));
    } catch (e, stackTrace) {
      debugPrint('💥 [JobCubit] Unexpected error: $e');
      debugPrint('📋 Stack trace: $stackTrace');
