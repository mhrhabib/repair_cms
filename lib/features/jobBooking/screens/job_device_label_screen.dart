...
class _JobDeviceLabelScreenState extends State<JobDeviceLabelScreen> {
  ...
  Future<void> _showPrinterSelection() async {
    debugPrint('🖨️ Opening printer selection dialog for label printing');

    final allPrinters = _settingsService.getAllPrinters();
    final List<PrinterConfigModel> labelPrinters = allPrinters['label'] ?? [];

    debugPrint('📊 Found ${labelPrinters.length} label printers');

    if (labelPrinters.isEmpty) {
      showCustomToast(
        'No label printers configured. Please configure a label printer in Settings > Printer Settings',
        isError: true,
      );
      return;
    }
    ...
    if (selectedPrinter != null && mounted) {
      debugPrint('🎯 User selected: ${selectedPrinter.printerBrand} label printer');
      _printLabel(selectedPrinter);
    }
  }

  /// Print label with selected printer
  Future<void> _printLabel(PrinterConfigModel printer) async {
    debugPrint('🚀 Starting label print job with ${printer.printerBrand}');
    debugPrint('📏 Label size: ${printer.labelSize?.width}mm x ${printer.labelSize?.height}mm');

    showCustomToast('Label print functionality will be implemented here', isError: false);

    // TODO: Implement actual label printing based on printer type and label size
    // final labelSize = printer.labelSize;
    // if (printer.printerBrand == 'Brother') {
    //   // Use Brother printer SDK
    // } else if (printer.printerBrand == 'Dymo') {
    //   // Use Dymo printer SDK
    // } else if (printer.printerBrand == 'Xprinter') {
