...
class _JobBookingStartBookingJobScreenState
  ...
  String _getUserId() {
    // Placeholder: infer userId from storage/user provider if available
  ...
  void _resetLocalChanges() {
    // Reset local UI state and notify cubit
    if (!mounted) return;
    setState(() {
      _selectedBrand = '';
    ...
    // Clear device brand in JobBookingCubit so device info doesn't persist
    try {
