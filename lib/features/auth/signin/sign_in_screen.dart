...
class _SignInScreenState extends State<SignInScreen> {
  ...
  void _checkBiometricOnStart() async {
    try {
      ...
      // Automatically trigger biometric authentication if enabled and not yet attempted
      if (_showBiometricOption && !_hasAttemptedAutoAuth) {
        _hasAttemptedAutoAuth = true;
        // Small delay to ensure the screen is fully rendered and ready for the system prompt
        Future.delayed(const Duration(milliseconds: 650), () {
          if (mounted && !_biometricLoginInProgress) {
            _authenticateWithBiometric(isAuto: true);
          }
        });
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    if (!_hasCheckedBiometric) {
      return Scaffold(
        ...
        body: Center(child: CupertinoActivityIndicator(color: AppColors.fontSecondaryColor, radius: 16)),
      );
    }
      ...
      body: Stack(
        children: [
          ...
          if (_biometricLoginInProgress)
            Positioned.fill(
              child: SizedBox(
                child: Center(child: CupertinoActivityIndicator(color: AppColors.fontSecondaryColor, radius: 16)),
              ),
            ),
        ],
      ),
                      ...
                      return CustomButton(
                        ...
                        child: state is SignInLoading
                            ? SizedBox(
                                height: 20,
                                width: 20,
                                child: CupertinoActivityIndicator(color: AppColors.whiteColor, radius: 16),
                              )
                            : null,
                      );
                    },
  ...
  Future<void> _authenticateWithBiometric({bool isAuto = false}) async {
    try {
      ...
      if (didAuthenticate) {
        ...
        if (credentials['email'] != null && credentials['password'] != null) {
          ...
          context.read<SignInCubit>().login(credentials['email']!, credentials['password']!);
        } else {
          SnackbarDemo(message: 'Credentials not found').showCustomSnackbar(context);
        }
      } else {
        debugPrint('❌ Authentication failed or canceled');
        // Only show snackbar if user explicitly tried (though we removed the button, keeping logic clean)
        if (!isAuto) {
